/* Kleegr — Voice note + File Attachment for GHL Internal Comments v34
 * Attachments are STAGED (not auto-sent). User can add text + voice + file, then send.
 * Audio player only for audio files, not PDFs/images/docs. */
(function kleegrVoiceComment() {
  ...
  var deletedUrls={};
  var stagedFile=null; // file waiting to be sent
  ...
  function uploadToServer(file,isVoice,noteText,statusCb){
    ...
    if(contactId)fd.append("contactId",contactId);if(conversationId)fd.append("conversationId",conversationId);if(locationId)fd.append("locationId",locationId);
  ...
  function showStagedFile(file){
    ...
    bar.innerHTML='<span>\uD83D\uDCCE</span><span class="klg-staged-name">'+escHtml(file.name)+'</span><span style="opacity:.6;font-size:11px">'+(file.size>1048576?(file.size/1048576).toFixed(1)+' MB':(file.size/1024).toFixed(0)+' KB')+'</span>';
    var rm=document.createElement("button");rm.type="button";rm.style.cssText="border:none;background:transparent;cursor:pointer;color:#dc2626;font:700 14px system-ui;padding:2px 4px";rm.textContent="\u2715";rm.title="Remove attachment";
    ...
    // insert the bar above the footer
    var card=input;for(var i=0;i<6&&card;i++){if(card.getBoundingClientRect().height>100)break;card=card.parentElement;}
    if(card){card.insertBefore(bar,card.lastElementChild);}else{input.parentElement.insertBefore(bar,input);}
  }
  function removeStagedBar(){var b=stagedBar();if(b)b.remove();}
  function escHtml(s){var d=document.createElement("div");d.textContent=s;return d.innerHTML;}
  function sendStagedFile(){
    if(!stagedFile)return;
    var f=stagedFile;var noteText=readComposerNote();
    stagedFile=null;removeStagedBar();
    clearComposer();
  ...
  function renderClip(target){var w=target||clipWrap();if(!w)return;w.innerHTML="";var inp=document.createElement("input");inp.type="file";inp.multiple=false;inp.style.display="none";inp.id="kleegr-file-input";
    inp.addEventListener("change",function(){if(inp.files&&inp.files[0]){showStagedFile(inp.files[0]);inp.value="";}});
    var btn=document.createElement("button");btn.type="button";btn.title="Attach a file";
    ...
    btn.innerHTML=clipSvg(AMBER);btn.addEventListener("mouseenter",function(){btn.style.background="rgba(180,83,9,0.10)"});btn.addEventListener("mouseleave",function(){btn.style.background="transparent"});btn.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();inp.click()});w.appendChild(inp);w.appendChild(btn);}

  // ---- DRAG AND DROP ----
  function setupDragDrop(){
    var input=activeInternalInput();if(!input)return;
    var card=input;for(var i=0;i<8&&card;i++){if(card.getBoundingClientRect().height>80)break;card=card.parentElement;}
    if(!card||card.__klgDrop34)return;card.__klgDrop34=true;
    card.style.position="relative";var overlay=null;
    ...
    card.addEventListener("drop",function(e){e.preventDefault();e.stopPropagation();if(overlay){overlay.remove();overlay=null;}
      var files=e.dataTransfer&&e.dataTransfer.files;if(files&&files.length){showStagedFile(files[0]);}
    });
  }
  ...
  function placeWrap(){if(recording)return;var w=wrap();var cw=clipWrap();var input=activeInternalInput();if(!input){if(w)w.remove();if(cw)cw.remove();return;}var footer=footerFrom(input);if(!footer){if(w)w.remove();if(cw)cw.remove();return;}if(!w){w=document.createElement("span");w.id="kleegr-voice-wrap";w.style.cssText="display:inline-flex;align-items:center;vertical-align:middle";renderIdle(w);}if(!cw){cw=document.createElement("span");cw.id="kleegr-clip-wrap";cw.style.cssText="display:inline-flex;align-items:center;vertical-align:middle";renderClip(cw);}var send=footer.querySelector("#conv-send-button-simple,[data-testid='send-button'],.conv-send-button,button[type='submit'],[id*='send-button']");var lg=footer.firstElementChild;var tgt,bef;if(lg&&send&&!lg.contains(send)){tgt=lg;bef=lg.firstChild}else{tgt=footer;bef=footer.firstChild}if(w.parentNode!==tgt){try{tgt.insertBefore(w,bef)}catch(e){footer.insertBefore(w,footer.firstChild)}}if(cw.parentNode!==tgt){try{tgt.insertBefore(cw,w.nextSibling)}catch(e){}}setupDragDrop();}

  // ---- AUDIO PLAYER (only for actual audio files) ----
