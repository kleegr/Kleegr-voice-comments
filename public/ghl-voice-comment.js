/* Kleegr — Voice note + File Attachment for GHL Internal Comments v32
 * Adds: paperclip button for click-to-attach, drag-and-drop on composer. */
(function kleegrVoiceComment() {
  ...
  var USERID_KEY="kleegr_voice_ghl_user_id";var _resolving=false;
  ...
  function uploadFile(file, isVoice, statusCb) {
    ...
    var composerNote=readComposerNote();
    if(composerNote)fd.append("note",composerNote);
    ...
    fetch(ENDPOINT,{method:"POST",body:fd}).then(function(r){return r.json().then(function(j){return{ok:r.ok,j:j}})}).then(function(res){
      if(res.j&&res.j.success){
        if(composerNote)clearComposer();
  ...
  // ---- PAPERCLIP (file attach) UI ----
  function renderClip(target){
    var w=target||clipWrap();if(!w)return;w.innerHTML="";
    var inp=document.createElement("input");inp.type="file";inp.multiple=false;inp.style.display="none";inp.id="kleegr-file-input";
    inp.addEventListener("change",function(){if(inp.files&&inp.files[0]){uploadFile(inp.files[0],false,function(t,c,ms){renderStatus(t,c,ms)});inp.value="";}});
  ...
  // ---- DRAG AND DROP on Internal Comment composer ----
  var dropSetUp=false;
  function setupDragDrop(){
    if(dropSetUp)return;
    var input=activeInternalInput();if(!input)return;
    // find the composer card (parent with position relative)
    var card=input;for(var i=0;i<8&&card;i++){if(card.getBoundingClientRect().height>80){break;}card=card.parentElement;}
    if(!card||card.__klgDrop)return;
    card.__klgDrop=true;dropSetUp=true;
    card.style.position="relative";
    var overlay=null;
    card.addEventListener("dragenter",function(e){e.preventDefault();e.stopPropagation();if(!overlay){overlay=document.createElement("div");overlay.className="klg-dropzone";overlay.textContent="Drop file to attach as internal comment";card.appendChild(overlay);}});
    ...
    card.addEventListener("drop",function(e){e.preventDefault();e.stopPropagation();if(overlay){overlay.remove();overlay=null;}
      var files=e.dataTransfer&&e.dataTransfer.files;
      if(files&&files.length){
        uploadFile(files[0],false,function(t,c,ms){renderStatus(t,c,ms)});
      }
    });
  }
  ...
  function placeWrap(){if(recording)return;var w=wrap();var cw=clipWrap();var input=activeInternalInput();if(!input){if(w)w.remove();if(cw)cw.remove();return;}
    var footer=footerFrom(input);if(!footer){if(w)w.remove();if(cw)cw.remove();return;}
    // mic button
    ...
    var lg=footer.firstElementChild;var tgt,bef;
    if(lg&&send&&!lg.contains(send)){tgt=lg;bef=lg.firstChild}else{tgt=footer;bef=footer.firstChild}
    if(w.parentNode!==tgt){try{tgt.insertBefore(w,bef)}catch(e){footer.insertBefore(w,footer.firstChild)}}
    if(cw.parentNode!==tgt){try{tgt.insertBefore(cw,w.nextSibling)}catch(e){}}
    setupDragDrop();
  }
  var AUDIO_RE=/(\.webm|\.ogg|\.oga|\.mp3|\.m4a|\.wav)(\?|$)/i;
  function isAudioHref(href){if(!href)return false;if(AUDIO_RE.test(href))return true;if(/filesafe\.space\/[^\s]*\/media\//i.test(href))return true;return false;}
  ...
  function hideVoiceMessage(chip,href){if(href)deletedUrls[href]=true;var node=chip;for(var i=0;i<20&&node&&node!==document.body;i++){try{var cs=window.getComputedStyle(node);var bg=cs.backgroundColor||"";if(bg&&bg!=="rgba(0, 0, 0, 0)"&&bg!=="transparent"&&bg!=="rgb(255, 255, 255)"&&bg.indexOf("255, 255, 255")===-1){var target=node;for(var j=0;j<3;j++){if(target.parentElement&&target.parentElement!==document.body)target=target.parentElement;}target.style.display="none";return;}}catch(e){}node=node.parentElement;}node=chip;for(var k=0;k<8&&node;k++){node=node.parentElement;}if(node)node.style.display="none";}
  function cleanEmSpaces(parent){var tw=document.createTreeWalker(parent,NodeFilter.SHOW_TEXT,null);var tn;while((tn=tw.nextNode())){var v=tn.nodeValue||"";if(v.indexOf("\u2003")>-1){tn.nodeValue=v.replace(/[\u2003]+/g," ").trim();}}}
  ...
  var ticking=false;function tick(){if(ticking)return;ticking=true;try{placeWrap();upgradeAudioComments()}catch(e){}ticking=false;}setInterval(tick,1000);tick();
})();
