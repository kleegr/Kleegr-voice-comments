 ...
 * Version 36 — Fix: fresh file input on every clip click, no URL in file attachment text.
 */
(function kleegrVoiceComment() {
  ...
  var deletedUrls={},stagedFile=null;
  ...
  function showStagedFile(file){
    ...
    bar.innerHTML='<span>\uD83D\uDCCE</span><span class="klg-staged-name">'+escHtml(file.name)+'</span><span style="opacity:.6;font-size:11px">'+sz+'</span>';
    ...
    var sb=document.createElement("button");sb.type="button";sb.title="Send with attachment";
    ...
    sb.addEventListener("click",function(e){e.preventDefault();e.stopPropagation();sendStagedFile()});
    ...
    console.log("[kleegr-voice] staged:",file.name,sz);
  }
  ...
  /* CLIP BUTTON — creates a FRESH file input on every click (appended to body, immune to GHL re-renders) */
  function renderClip(target){
    ...
    btn.addEventListener("click",function(e){
      ...
      // Fresh input every click — guaranteed to work for repeat attaches
      var inp=document.createElement("input");inp.type="file";inp.multiple=false;
      inp.style.cssText="position:fixed;top:-9999px;left:-9999px;opacity:0";
      document.body.appendChild(inp);
      inp.addEventListener("change",function(){if(inp.files&&inp.files[0]){showStagedFile(inp.files[0])}inp.remove()});
      // also remove if user cancels the picker
      setTimeout(function(){if(document.body.contains(inp))inp.remove()},120000);
      inp.click();
    });
    w.appendChild(btn);
  }
  ...
  var ticking=false;function tick(){if(ticking)return;ticking=true;try{placeWrap();upgradeAudioComments()}catch(e){console.error("[kleegr-voice] tick:",e)}ticking=false}setInterval(tick,1000);tick();
})();
